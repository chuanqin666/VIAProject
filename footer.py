...
with open('val_footers/via_export_json.json') as json_file:
...
for z in range(len(keys)):
    ...
    file_path = 'val_footers/' + filename_value[0].strip(".\\")
    ...
    for i in range(len(xy_sorted)):
        ...
        # If two rectangles belong to the same outer rectangle. #
            ...
            if (xy_sorted[i - 1][1][5] > xy_sorted[i][1][1]
                ...
                >= xy_sorted[i][1][1]) \
                    and (xy_sorted[i - 1][1][4] <= xy_sorted[i][1][0]
                         or xy_sorted[i][1][4] <= xy_sorted[i - 1][1][0]):
            ...
            """
                # Check which rectangle's right-bottom Y is larger. #
...
GET_HTML = "footer.html"
f = open(GET_HTML, 'w')
f.write(index_page)
f.close()

webbrowser.open("footer.html")
